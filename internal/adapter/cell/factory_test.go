...
func TestFactoryは完成した配下EntityからCellを生成する(t *testing.T) {
	...
	target, _ := domain.NewContainer(nil, "app", domain.Target)
	dependency, _ := domain.NewContainer(nil, "db", domain.Dependency)
	...
	if cell.ContainerResourceName(cell.Containers.Items[0]) == "app" {
		t.Fatal("target container must have a cell-specific name")
	}
	if cell.ContainerResourceName(cell.Containers.Items[1]) != "db" {
