...
description: Use when a user wants to solve a problem, fix a bug, or add a feature and the outcome should be a GitHub issue created from the current conversation after design and implementation planning.
---

# request-to-issue

Create a GitHub issue from the current conversation by running design, planning, and issue drafting in order.
...
- **REQUIRED SUB-SKILL:** Use `superpowers:writing-plans` after the design spec is approved
...
4. Follow the `superpowers:brainstorming` flow to present the design, get approval, and write the spec to `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`.
5. Follow the `superpowers:writing-plans` flow to write the implementation plan to `docs/superpowers/plans/YYYY-MM-DD-<topic>.md`.
6. Convert the approved spec and plan into a GitHub issue body using the existing `takt-plan-to-issue` template and section rules.
7. Create the issue with `gh issue create`.

## Output Rules

- The spec file must exist before writing the implementation plan.
- The implementation plan must exist before creating the issue.
...
Create the issue only after the draft title and body are coherent with the approved spec and plan.
